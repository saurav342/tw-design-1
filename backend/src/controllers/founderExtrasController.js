...
const listExtras = (req, res) => {
  try {
    const items = listFounderExtras();
...
const getExtras = (req, res) => {
  ...
  try {
    const extras = getFounderExtras(founderId);
...
const upsertMarketplace = (req, res) => {
  const { founderId } = req.params;
  if (!canAccessFounderExtras(req, founderId)) {
  ...
  try {
    const extras = upsertMarketplaceListing(founderId, req.body ?? null);
    return res.status(200).json({ extras });
  } catch (error) {
...
const upsertSuccessFee = (req, res) => {
  const { founderId } = req.params;
  if (!canAccessFounderExtras(req, founderId)) {
  ...
  try {
    const extras = recordSuccessFeeRequest(founderId, req.body ?? null);
    return res.status(200).json({ extras });
  } catch (error) {
...
const createServiceRequest = (req, res) => {
  ...
  try {
    const extras = addServiceRequest(founderId, req.body ?? null);
