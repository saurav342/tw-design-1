...
const Founders = () => (
  <div className="mx-auto flex max-w-7xl flex-col gap-16 px-4 py-12 lg:px-8">
    <section className="grid gap-12 lg:grid-cols-[1fr_1fr] lg:items-center">
      <div className="rounded-3xl bg-lilac/70 p-8 shadow-sm shadow-[0_25px_65px_rgba(46,220,146,0.25)]">
        <SectionHeader
        ...
        <div className="mt-6 grid gap-4">
          {founderHighlights.valueProps.map((value) => (
            <div key={value.title} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm shadow-slate-200">
              <h3 className="text-lg font-semibold text-night">{value.title}</h3>
              <p className="mt-2 text-sm text-slate-600">{value.description}</p>
            </div>
          ))}
        </div>
        <div className="mt-6 flex flex-wrap gap-4">
          <Link to="/signup/founder" className="rounded-full bg-blossom px-6 py-3 text-sm font-semibold uppercase tracking-wide text-white shadow-lg shadow-[0_28px_75px_rgba(255,79,154,0.35)] hover:bg-royal">
      ...
      <div className="space-y-6">
        <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm shadow-slate-200">
          <h3 className="text-xl font-semibold text-night">Launch readiness score</h3>
          ...
          <ul className="mt-4 space-y-3 text-sm text-slate-600">
            ...
            <li className="flex items-start gap-3">
              <span className="mt-1 h-2 w-2 rounded-full bg-sunbeam" />
        ...
        <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm shadow-slate-200">
          <h3 className="text-xl font-semibold text-night">LaunchAndLift Studios</h3>
          <p className="mt-3 text-sm text-slate-600">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {founderHighlights.process.map((step) => (
          <div key={step.label} className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm shadow-slate-200">
            <span className="text-xs font-semibold uppercase tracking-[0.4em] text-sprout">
              {step.label}
            </span>
    ...
    <section className="rounded-3xl bg-gradient-to-r from-royal via-blossom to-sunbeam px-6 py-12 text-white shadow-2xl shadow-[0_30px_80px_rgba(247,201,72,0.28)]">
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        ...
        <Link to="/signup/founder" className="rounded-full bg-white px-6 py-3 text-sm font-semibold uppercase tracking-wide text-sunbeam hover:bg-mint hover:text-night">
          Submit your company
        </Link>
      </div>
    </section>
