...
const Signup = () => {
  ...
  return (
    <div className="bg-brand-muted/60 px-4 py-16">
      <div className="mx-auto max-w-6xl rounded-3xl border border-slate-100 bg-white shadow-2xl shadow-brand-muted/60">
        <div className="grid gap-10 p-10 md:grid-cols-[0.9fr_1.1fr] md:p-16">
          <div className="space-y-6">
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-lagoon">Activate access</p>
            <h1 className="text-3xl font-semibold text-brand-dark">Choose your LaunchAndLift onboarding path</h1>
            ...
            <div className="rounded-2xl bg-brand-muted p-5 text-sm text-slate-600">
              <p className="font-semibold text-brand-dark">What to expect</p>
          ...
          <div className="grid gap-6">
            {signupFlows.map((flow) => (
              <div
                key={flow.role}
                className="flex flex-col justify-between rounded-3xl border border-slate-100 bg-white p-6 shadow-sm shadow-slate-200 transition hover:-translate-y-1 hover:border-lagoon/60 hover:shadow-lagoon/30"
              >
                <div className="space-y-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-lagoon">{flow.eyebrow}</p>
                  <h2 className="text-2xl font-semibold text-brand-dark">{flow.title}</h2>
                ...
                <div className="mt-6">
                  <Link
                    to={flow.to}
                    className="inline-flex w-full items-center justify-center rounded-full bg-blaze px-6 py-3 text-sm font-semibold uppercase tracking-wide text-white shadow-lg shadow-blaze/40 transition hover:bg-sunset"
                  >
                    {flow.cta}
                  </Link>
                </div>
            ...
            <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-6 text-sm text-slate-500">
              <p className="font-semibold text-brand-dark">LaunchAndLift team member?</p>
              <p className="mt-2">
